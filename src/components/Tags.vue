    ...
    <div class="form-control form-control-tags">
        <div style="position:relative;left:0px;top:0px;min-width:100%;height:100%; padding: 11px; ">
            <select :placeholder="placeholder" style="position:absolute;left:0px;top:0px;width:100%;height:100%;opacity:0;"
                    v-on:change="select">
                <option v-for="(option,i) in domain" :key="i" v-bind:value="typeof option=='object'?option.id:option" :hidden="isSelected(typeof option=='object'?option.id:option)">{{getTextField(option.attributes, field.textField)}}</option>
                <option value="" hidden=""></option>
            </select>
            <template v-for="(option,i) in selected">
                <span :key="i" class="label label-tag" :value="option.value" style="position:relative;" v-on:click="remove">{{option.text}} <i class="glyphicon glyphicon-remove"></i></span>
                <i :key="i"> </i>
            </template>
        </div>
    </div>
        ...
        methods: {
            ...
            clickControl: function (event) {
                if(event.target.nodeName==='DIV') {
                    window.$(event.target.previousElementSibling).click();
                }
            },
            getValues: function() {
