<template>
  <div>
    ...
    <table v-if="tableMode" class="grid-table table table-hover">
      <caption>
        <loading v-if="value && value.loading"></loading>
      </caption>
      <thead>
    ...
    <slot v-if="!tableMode" name="header" v-bind:data="value" v-bind:options="options"></slot>
    <div v-if="!tableMode">
      <slot
        v-for="row in currentPage"
        v-bind:row="row"
